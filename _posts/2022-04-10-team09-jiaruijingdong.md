...
> Our project focuses on module weak supervision. And we are mainly interested in the area of zero-shot localization. We aimed at using CLIP as backbone because of its outstanding performance on zero shot classification tasks. We proved that extending class activation to CLIP can achieve localization prediction on novel datasets.
...
## Main Content
Our method heavily relies on CLIP with its outstanding performance on zero shot classification tasks. We would like to adapt other methods such as class activation/attention to CLIP like architectures so that we can achieve localization prediction on novel datasets. This page will introduce the problem statement, related literature, pipeline, dataset and evaluation metric and experiment analysis.

## Background and Problem Statement

Object localization is a hot research area. Supervised localization methods, for example YOLO and Faster-RCNN are famous for high obeject detection accuracy. But localization ground truth annotation is costly for human. This leads to the emergence of weakly supervised localization. With only image-level supervision, it can still achieve good detection accuracy. Models in these two research areas normally requires training on the predetermined objects to learn corresponding features. When there are new object categories, the model may not be generalize enough to maintain the same performance accuracy. Zero shot localization[7, 8] is good at localizing novel categories without training example. Ideally, in industry, it can save training time as well as annotation time, along with high localization accuracy. 

Our pipeline focuses on zero shot object localization, using CLIP[2] and ScoreCAM[3]. CLIP[2] is famous for its classification generality in zero-shot way. It is used for recognizing hidden features. Then the pipeline transfers the information to ScoreCAM[3] to feature visualization and localization. The detailed code can be found in [link](https://arxiv.org/pdf/2103.00020.pdf). A sample code snippet for users to play with different label and image is : [https://colab.research.google.com/drive/1kavCQkJZNe_vSMmLf587QhvDFILRiO8a?usp=sharing](https://colab.research.google.com/drive/1kavCQkJZNe_vSMmLf587QhvDFILRiO8a?usp=sharing)
