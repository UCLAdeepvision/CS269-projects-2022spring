...
We have mentioned FlowStyle model in the related work that it has better performance than existing UNET based models in terms of image virtual tryon. Existing Video virtual tryon models make use of the UNET model as the core to perform the  cloth warping. Hence we decided to use the latest FlowStyle model as the core of our model to perform the image virtual tryon. For the baseline model, we have split the input videos into multiple image frames. These input image frames are passed to the FlowStyle model and an output frame is received that has the new cloth on the image. All these output image frames are then combined into corresponding output videos. The FlowStyle model, as we have discussed earlier, estimates the dense pose of the person and warps the cloth to that pose very well and hence image outputs will be of very high qulaity for eeach frame. We have observed that these videos were very smooth compared to what we have expected. The main reason might be due to the fact that these videos were not very fast moving and had perfect backgrounds that helped the output videos to be smooth. Then we have tried with videos that are not very ideal with different backgrounds and speeds. We found that the output videos are now having many artefacts and there is a scope of improvement. For this reason we have come up with the following approaches to improve the performance of our model.

![image-20220609194336211](/Users/vishnu1911/Library/Application Support/typora-user-images/image-20220609194336211.png)

### Using Flow - Trying to improve overall flow of the model:

To improve the videos generated using the baseline method given above, we have tried out Flow which is generated by Flownet 2 model between images. Theoretically flow should improve the video quality as we do not expect the image to make drastic movement in the virtual tryon task. Hence we calculated the flow between two consective input images. This flow is then used to resample the target output. This resampled target output is then clubbed with the output image generated from the FlowStyle model so that the cloth region of the image is picked up from the output image and the rest of the part is mainly from the flow sampling. This helps to improve the flow of the background along with the person and the cloth will be generated as per the densepose recognized by the FlowStyle model.
